            ...
            <div class="card">
              <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <form method="GET" class="form-inline">
                  <input type="text" name="keyword" value="<?= htmlspecialchars($keyword) ?>" class="form-control mr-2" placeholder="Cari nama jabatan" />
              ...
              <div class="card-body">
                ...
                <form method="POST" class="form-inline mb-3">
                  <div class="form-group mr-2" style="flex: 1;">
                    <input type="text" name="nama_jabatan" class="form-control w-100" placeholder="Nama jabatan baru" required />
                  </div>
                ...
                if (isset($_POST['simpan'])) {
                  ...
                  $insert = mysqli_query($conn, "INSERT INTO jabatan (nama_jabatan) VALUES ('$nama_jabatan')");
                  if ($insert) {
                    $_SESSION['flash_message'] = "Jabatan berhasil ditambahkan.";
                    echo "<script>location.href='jabatan.php';</script>";
                    exit;
                  } else {
                    echo "<div class='alert alert-danger'>Gagal menambahkan jabatan.</div>";
                  }
                }
                ...
                // Hitung total data
                $totalQuery = "SELECT COUNT(*) as total FROM jabatan";
                if (!empty($keyword)) {
                  $keywordEscaped = mysqli_real_escape_string($conn, $keyword);
                  $totalQuery .= " WHERE nama_jabatan LIKE '%$keywordEscaped%'";
                }
                ...
                if (!empty($keyword)) {
                  $query .= " WHERE nama_jabatan LIKE '%$keywordEscaped%'";
                }
                $query .= " ORDER BY nama_jabatan ASC LIMIT $offset, $limit";
                $result = mysqli_query($conn, $query);
                $no = $offset + 1;
                ?>
                      ...
                      if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                          ...
                          echo "<td class='text-center'>
                                  <a href='jabatan.php?hapus={$row['id']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Hapus jabatan ini?\")'>
