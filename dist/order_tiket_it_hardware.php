            ...
            <div class="card-body">
              ...
              <div class="tab-content mt-4" id="myTabContent">
                ...
                <div class="tab-pane fade show active" id="order" role="tabpanel">
                  <form method="POST" action="simpan_tiket_it_hardware.php" id="form-order-hardware">
                          ...
                          if ($status_validasi == 'Belum Validasi') {
                            return "
                              <div class='d-flex gap-1'>
                                <form method='post' action='validasi_tiket.php' class='form-validasi' style='display:inline-block; margin-right: 5px;'>
                                  <input type='hidden' name='tiket_id' value='$id'>
                                  <input type='hidden' name='ajax' value='1'>
                                  <button type='submit' name='validasi' class='btn btn-sm btn-success'>Terima</button>
                                </form>
                                <form method='post' action='validasi_tiket.php' class='form-validasi' style='display:inline-block;'>
                                  <input type='hidden' name='tiket_id' value='$id'>
                                  <input type='hidden' name='ajax' value='1'>
                          ...
                          if ($status_validasi == 'Diterima' && $status_utama_low == 'diproses') {
                            return "
                              <form method='post' action='selesai_tiket.php' class='form-selesai'>
                                <input type='hidden' name='tiket_id' value='$id'>
                                <input type='hidden' name='tipe' value='hardware'>
                                <input type='hidden' name='ajax' value='1'>
...
<script>
  $(document).ready(function() {
    ...
    // AJAX Submit Tiket Hardware
    $('#form-order-hardware').on('submit', function(e) {
      e.preventDefault();
      var form = $(this);
            ...
            success: function(response) {
              if(response.status == "success") {
                Swal.fire({
                    icon: 'success',
                    title: 'SUCCESS!',
                    html: 'Hardware Service Request berhasil disimpan & dikirim ke Telegram & WA.<br><div class="sr-number">' + response.nomor_tiket + '</div>',
