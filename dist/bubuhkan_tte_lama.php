    ...
    try {
        ...
        for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
            ...
            if ($pageNo == $target_page) {
                ...
                // LAYER 1: Hidden token (extractable font size)
                $pdf->SetFont('Arial', '', 1);  // Changed from 0.1 to 1 for extraction
        ...
        // LAYER 2: Enhanced PDF Metadata (STANDAR)
        $pdf->SetCreator('FixPoint TTE System v2.0 - Digital Signature Platform');
        $pdf->SetAuthor($tte['nama'] . ' (NIK: ' . $tte['nik'] . ')');
        $pdf->SetTitle('Dokumen Resmi Bertanda Tangan Digital - ' . pathinfo($filename, PATHINFO_FILENAME));
        $pdf->SetSubject('TTE-TOKEN:' . $tte['token'] . ' | Signed Document');
        
        // Keywords for search and indexing (STANDAR)
        $keywords = implode(', ', array_filter([
            'TTE', 'Digital Signature', 'Tanda Tangan Elektronik',
            $tte['nama'], 'NIK:' . $tte['nik'],
            $tte['jabatan'] ?? null, $tte['unit_kerja'] ?? null,
            'TOKEN:' . substr($tte['token'], 0, 16),
            'DATE:' . date('Y-m-d'), 'FixPoint', 'Verified', 'Official Document'
        ]));
...
function processWordCustomPosition($filepath, $qr_file, $position_x, $position_y, $tte, $filename) {
    ...
    try {
        ...
        // LAYER 2: Enhanced DOCX Metadata (STANDAR)
        $docInfo = $phpWord->getDocInfo();
        $docInfo->setCreator('FixPoint TTE System v2.0 - Digital Signature Platform');
        $docInfo->setLastModifiedBy($tte['nama'] . ' (NIK: ' . $tte['nik'] . ')');
        $docInfo->setTitle('Dokumen Resmi Bertanda Tangan Digital - ' . pathinfo($filename, PATHINFO_FILENAME));
        $docInfo->setSubject('Digital Signature Document');
        $docInfo->setDescription('TTE-TOKEN:' . $tte['token'] . ' | Signed by: ' . $tte['nama'] . ' | NIK: ' . $tte['nik'] . ' | Date: ' . date('Y-m-d H:i:s'));
        
        // Keywords for search (STANDAR)
        $keywords = implode(', ', array_filter([
            'TTE', 'Digital Signature', 'Tanda Tangan Elektronik',
            $tte['nama'], 'NIK:' . $tte['nik'],
            $tte['jabatan'] ?? null, $tte['unit_kerja'] ?? null,
        ...
        $docInfo->setCategory('Official Document - Digitally Signed');
    ...
    if (!isset($_FILES['dokumen']) || $_FILES['dokumen']['error'] !== UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $temp_filepath)) {
                ...
                // QR Content: Plain text dengan info lengkap TTE (bukan URL)
                ...
                // Info perusahaan/instansi
                if ($perusahaan_info) {
                    $qr_content .= "PERUSAHAAN/INSTANSI:\n";
                    $qr_content .= $perusahaan_info['nama_perusahaan'] . "\n";
                    if (!empty($perusahaan_info['kota']) || !empty($perusahaan_info['provinsi'])) {
                        $lokasi = array_filter([
                            $perusahaan_info['kota'], 
                            $perusahaan_info['provinsi']
                        ]);
                        $qr_content .= implode(', ', $lokasi) . "\n";
                    }
                    $qr_content .= "\n";
                }
                ...
                // Generate QR Code dengan plain text content
