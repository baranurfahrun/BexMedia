...
// Ambil data maintenance + user + barang IT
$query = mysqli_query($conn, "
    SELECT m.*, u.nik, u.nama_lengkap as nama, u.jabatan, u.unit_kerja, 
           b.no_barang, b.nama_barang, b.kategori, b.merk, b.lokasi
    FROM maintanance_rutin m
    JOIN users u ON m.user_id = u.id
    LEFT JOIN data_barang_it b ON m.barang_id = b.id
    WHERE m.id = '$id'
");
...
<head>
  ...
  <title>Cetak Kartu Maintenance IT</title>
  <style>
    body { 
...
<div class="ticket">
  ...
  <div class="title">KARTU MAINTENANCE RUTIN (IT)</div>
  ...
  <div class="title" style="font-size:11px;">DETAIL BARANG IT</div>
  <div class="info">
    <div><span class="label">No. Barang</span>: ' . htmlspecialchars($data['no_barang']) . '</div>
    <div><span class="label">Nama Barang</span>: ' . htmlspecialchars($data['nama_barang']) . '</div>
    <div><span class="label">Kategori</span>: ' . htmlspecialchars($data['kategori']) . '</div>
  ...
  <div class="catatan"><strong>Catatan Teknisi:</strong><br>' . (!empty($data['catatan']) ? nl2br(htmlspecialchars($data['catatan'])) : '-') . '</div>
...
$dompdf->add_info('Title', 'Kartu Maintenance IT - ' . $data['no_barang']);

$dompdf->render();

// Watermark dihapus sesuai permintaan

$dompdf->stream('kartu_maintenance_it_' . $data['id'] . '.pdf', ['Attachment' => false]);
?>
