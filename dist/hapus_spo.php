...
// Cek akses user
$current_file = 'spo.php'; // File induk
...
if($id > 0) {
    ...
    $qFile = mysqli_query($conn, "SELECT file_path FROM spo WHERE id = $id LIMIT 1");
    if($qFile && mysqli_num_rows($qFile) > 0) {
        ...
        // Hapus dari database
        if(mysqli_query($conn, "DELETE FROM spo WHERE id = $id")) {
            $_SESSION['flash_message'] = 'SPO berhasil dihapus.';
        } else {
