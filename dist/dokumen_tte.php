...
if (isset($_POST['send_email']) && isset($_POST['doc_id']) && isset($_POST['email_recipients'])) {
    ...
    $qDoc = $conn->prepare("
        ...
        WHERE dsl.id = ? AND dsl.user_id = ?
    ");
    $qDoc->bind_param("ii", $doc_id, $user_id);
    $qDoc->execute();
    ...
    header("Location: dokumen_tte.php");
    exit;
}
...
if (isset($_POST['delete_doc']) && isset($_POST['doc_id'])) {
    ...
    $qFile = $conn->prepare("SELECT document_name FROM tte_document_log WHERE id = ? AND user_id = ?");
    $qFile->bind_param("ii", $doc_id, $user_id);
    $qFile->execute();
    ...
    if ($fileData) {
        ...
        $qDelete = $conn->prepare("DELETE FROM tte_document_log WHERE id = ? AND user_id = ?");
        $qDelete->bind_param("ii", $doc_id, $user_id);
...
$conditions = ["dsl.user_id = ?"];
$params = [$user_id];
$types = "i";

// Add search condition
if (!empty($search)) {
    $conditions[] = "(dsl.document_name LIKE ? OR dsl.document_hash LIKE ?)";
    $params[] = $searchParam;
    $params[] = $searchParam;
    $types .= "ss";
}
...
$conditions[] = "DATE(dsl.signed_at) BETWEEN ? AND ?";
$params[] = $date_from;
$params[] = $date_to;
$types .= "ss";
...
// Count documents
$qCount = $conn->prepare("SELECT COUNT(*) as total FROM tte_document_log dsl $whereClause");
...
$qDocs = $conn->prepare("
    SELECT dsl.*, tu.nama, tu.nik, tu.jabatan, tu.token 
    FROM tte_document_log dsl
    JOIN tte_user tu ON dsl.tte_token = tu.token
    $whereClause
...
/* Table Compact Design with Horizontal Scroll */
.table-container {
    overflow-x: auto;
...
<div class="section-body">
    ...
    <div class="row">
        ...
        <div class="col-lg-3 col-md-6">
            <div class="stats-card" style="border-left-color: #10b981;">
                <h3><?= number_format($stats['signing_days'] ?? 0) ?></h3>
                <p><i class="fas fa-calendar-check text-success"></i> Hari Menandatangani</p>
            </div>
        </div>
        <div class="col-lg-3 col-md-6">
            <div class="stats-card" style="border-left-color: #3b82f6;">
                ...
                <p><i class="fas fa-clock text-info"></i> Tanda Tangan Pertama</p>
            </div>
        </div>
        <div class="col-lg-3 col-md-6">
            <div class="stats-card" style="border-left-color: #f59e0b;">
                ...
                <p><i class="fas fa-check-circle text-warning"></i> Tanda Tangan Terakhir</p>
            </div>
        </div>
    </div>
    ...
    <div class="search-box">
        <form method="GET" class="row align-items-center" id="filterForm">
            ...
            <div class="col-md-2">
                <input type="date" name="date_from" id="date_from" class="form-control" value="<?= htmlspecialchars($date_from) ?>" title="Tanggal Dari">
            </div>
            <div class="col-md-2">
                <input type="date" name="date_to" id="date_to" class="form-control" value="<?= htmlspecialchars($date_to) ?>" title="Tanggal Sampai">
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                <a href="dokumen_tte.php" class="btn btn-secondary btn-block">
        ...
        <div class="mt-3 d-flex justify-content-between align-items-center">
            <div>
                <small class="text-muted">
                    ...
                    Menampilkan data periode: <strong><?= date('d/m/Y', strtotime($date_from)) ?></strong> s/d <strong><?= date('d/m/Y', strtotime($date_to)) ?></strong>
                </small>
            </div>
            <div class="btn-group btn-group-sm" role="group">
                ...
                <button type="button" class="btn btn-outline-primary" onclick="setDateFilter('all')">
    ...
    <div class="card">
        <div class="card-header">
            <h4><i class="fas fa-list"></i> Daftar Dokumen (<?= number_format($totalDocs) ?>)</h4>
        ...
        <div class="card-body p-0">
                ...
                <table class="table table-hover doc-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="font-weight: 600; color: #334155;"><?= htmlspecialchars($doc['nama']) ?></td>
                            <td style="color: #64748b;"><?= htmlspecialchars($doc['nik']) ?></td>
                            <td>
                ...
                <nav>
                    <ul class="pagination justify-content-center mb-0">
                        ...
                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                            <a class="page-link" href="?page=<?= $page - 1 ?><?= $urlQuery ?>">
                        ...
                        <li class="page-item <?= $page == $i ? 'active' : '' ?>">
                            <a class="page-link" href="?page=<?= $i ?><?= $urlQuery ?>">
                                <?= $i ?>
                            </a>
                        </li>
                        <?php endfor; ?>
            ...
            <div class="empty-state">
                ...
                <p>Anda belum menandatangani dokumen apapun</p>
                <a href="bubuhkan_tte.php" class="btn btn-primary mt-3">
                    <i class="fas fa-plus"></i> Tandatangani Dokumen
                </a>
            </div>
            <?php endif; ?>
        </div>
...
function setDateFilter(period) {
    ...
    switch(period) {
        ...
        case 'month':
            ...
            break;
        case 'all':
            // Set to very old date to get all records
            dateFrom = '2020-01-01';
            dateTo = today.toISOString().split('T')[0];
            break;
    }
