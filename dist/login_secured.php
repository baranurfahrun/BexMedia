...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['email'], $_POST['password'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (!checkRateLimit($ip_address, $conn)) {
            $notif = "Terlalu banyak percobaan login. Silakan coba lagi dalam 15 menit.";
        } else {
            ...
            if ($email === "" || $password === "") {
                ...
                if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    if ($stmt->num_rows === 1) {
                        ...
                        if ($status != 'active') {
                            $notif = "Login gagal. Periksa kredensial Anda."; // Generic message
                            recordLoginAttempt($ip_address, $email, 0, $conn);
                        ...
                        // Generic message agar tidak membocorkan informasi
    ...
    <form method="POST" action="proses_register.php" class="modal-content" autocomplete="off">
      ...
      <div class="modal-body">
        <div class="form-row">
          <div class="form-group col-md-6">
            <label>NIK</label>
            <input type="text" name="nik" class="form-control" required maxlength="16" pattern="[0-9]{16}" title="NIK harus 16 digit angka">
          </div>
          <div class="form-group col-md-6">
