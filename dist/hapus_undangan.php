...
// Cek akses user
$current_file = 'rapat_bulanan.php'; // File induk
...
if($id > 0) {
    ...
    $qFile = mysqli_query($conn, "SELECT file_path FROM undangan_rapat WHERE id = $id LIMIT 1");
    if($qFile && mysqli_num_rows($qFile) > 0) {
        ...
        // Hapus dari database
        if(mysqli_query($conn, "DELETE FROM undangan_rapat WHERE id = $id")) {
            $_SESSION['flash_message'] = 'Undangan berhasil dihapus.';
        } else {
